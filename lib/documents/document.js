...
export class Document extends BasePermissionedDocument {
  ...
  static _restrictQueryForAnonymousUser(query, permissions) {
    let allowedVisibilityLevels;
    if (this._isDirectDocumentQuery(query)) {
      allowedVisibilityLevels = [Document.VISIBILITY_LEVELS.PUBLIC, Document.VISIBILITY_LEVELS.LISTED];
    }
    else {
      allowedVisibilityLevels = [Document.VISIBILITY_LEVELS.LISTED];
    }
  ...
  // We specially handle public and listed documents. We return public documents only if
  // the query explicitly specifies them by their IDs.
  static _restrictQuery(query, permissions, user) {
    assert(user);
    ...
    return {
      $or: [
        super._restrictQuery(query, permissions, user),
        {
          visibility: {$in: allowedVisibilityLevels},
  ...
  _canUser(permissions, user) {
    ...
    // We specially handle public and listed documents. We check it in this way to handle
    ...
    // For public and listed documents, if no other permission check returned "true",
    // we check if any of the requested permissions are among default permissions.
  ...
  _canAnonymousUser(permissions) {
    ...
    // For public and listed documents, if no other permission check returned "true", we check
    // if any "view" permissions of the requested permissions are among default permissions.
