...
export class Document extends BasePermissionedDocument {
  ...
  static _isDirectDocumentQuery(query) {
    ...
    if (_.isObject(query._id) && query._id.$in && _.isArray(query._id.$in) && query._id.$in.length && _.every(query._id.$in, _.isString)) {
      return true;
    }
  ...
  static _getAllowedVisibilityLevels(query, existsQuery) {
    // If the query is just checking if documents exist, but not listing them (assumption here is
    // that such queries do not reveal IDs anything more then just trying to see if a particular
    // ID exists), or if the query uses explicitly specified IDs.
    if (existsQuery || this._isDirectDocumentQuery(query)) {
  ...
  static _restrictQueryForAnonymousUser(query, permissions, options) {
    const allowedVisibilityLevels = this._getAllowedVisibilityLevels(query, options && options.existsQuery);
  ...
  static _limitQuery(query, permissions, user, options) {
    assert(user);
    ...
    if (updatePermissions.length) {
      queries.push({
        $and: [
          super._limitQuery(query, updatePermissions, user, options),
          {
            publishedAt: null,
          },
        ],
    ...
    if (otherPermissions.length) {
      queries.push(super._limitQuery(query, otherPermissions, user, options));
      queries.push({
        visibility: {$in: allowedVisibilityLevels},
  ...
  static _limitQueryClassPermissions(query, permissions, user, options) {
    assert(user);
  ...
  // We specially handle public and listed documents. We do not have to differentiate between public
  // and listed documents in this method because we operate only on a concrete document here.
  _canAnonymousUser(permissions) {
    const allowedVisibilityLevels = [this.constructor.VISIBILITY_LEVELS.PUBLIC, this.constructor.VISIBILITY_LEVELS.LISTED];

    // An anonymous user can at most do only things a "view" role can.
    const anonymousPermissions = _.intersection(permissions, this.constructor.getRolePermissions(this.constructor.ROLES.VIEW));

    // "UPDATE" permission should not be among these permissions because we
    // are not handling published documents in this method.
    assert(!_.contains(anonymousPermissions, this.constructor.PERMISSIONS.UPDATE));

    return !!(_.contains(allowedVisibilityLevels, this.visibility) && _.intersection(this.defaultPermissions, anonymousPermissions).length);
  }

  // We specially handle published documents and do not allow "UPDATE" permission for them.
  // We specially handle public and listed documents. We do not have to differentiate between public
  // and listed documents in this method because we operate only on a concrete document here.
  _checkPermissions(permissions, user) {
    assert(user);

    const updatePermissions = _.intersection(permissions, [this.constructor.PERMISSIONS.UPDATE]);
    const otherPermissions = _.without(permissions, this.constructor.PERMISSIONS.UPDATE);

    const allowedVisibilityLevels = [this.constructor.VISIBILITY_LEVELS.PUBLIC, this.constructor.VISIBILITY_LEVELS.LISTED];

    if (updatePermissions.length) {
      if (!this.isPublished()) {
        if (super._checkPermissions(updatePermissions, user)) {
          return true;
        }
        if (_.contains(allowedVisibilityLevels, this.visibility) && _.intersection(this.defaultPermissions, updatePermissions).length) {
          return true;
        }
      }
    }
    if (otherPermissions.length) {
      if (super._checkPermissions(otherPermissions, user)) {
        return true;
      }
      if (_.contains(allowedVisibilityLevels, this.visibility) && _.intersection(this.defaultPermissions, otherPermissions).length) {
        return true;
      }
    }
  ...
  // on all documents for them. This is why there is no point in limiting the query based on
  // visibility and default permissions.
  _checkClassPermissions(permissions, user) {
    assert(user);

    // "UPDATE" permission is the only class-level permission from "permissions" the user has.
    if (!User.hasClassPermission(_.without(permissions, this.constructor.PERMISSIONS.UPDATE), user)) {
