...
export class Activity extends BaseDocument {
  ...
  static PUBLISH_FIELDS() {
    ...
    if (userId) {
      forUsers = {
        forUsers: {
          // We can use "$elemMatch" here in the projection, because
          // there is only one item in "forUsers" we want to keep.
          $elemMatch: {
            _id: userId,
          },
        },
  ...
  fields(fields) {
    return _.extend(fields, {
      ...
      data: {
        ...
        parentDocument: Activity.ReferenceField(Document, ['title'], false),
      },
    });
  },
});
