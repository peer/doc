...
export class BasePermissionedDocument extends BaseDocument {
  ...
  static restrictQuery(query, permissions, user, options) {
    ...
    if (!user) {
      return this._restrictQueryForAnonymousUser(query, permissions, options);
    }
    ...
    if (User.hasClassPermission(permissions, user)) {
      ...
      limitQuery = this._limitQueryClassPermissions(query, permissions, user, options);
    ...
    limitQuery = this._limitQuery(query, permissions, user, options);
  ...
  static existsAndCanUser(query, permissions, user) {
    // eslint-disable-next-line import/no-duplicates
    import {User} from '/lib/documents/user';
    ...
    return this.documents.exists(this.restrictQuery(query, permissions, user, {existsQuery: true}));
  }
  ...
  canUser(permissions, user) {
    ...
    // Does user have a class-level permission?
    if (User.hasClassPermission(permissions, user)) {
      if (this._checkClassPermissions(permissions, user)) {
        return true;
      }
    }

    if (this._checkPermissions(permissions, user)) {
      return true;
    }
