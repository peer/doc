...
export class BaseDocument extends Document {
  ...
  static restrictQuery(query, permissions, user, instancePermissionQuery) {
    ...
    // Does user have a instance-level permission?
    if (instancePermissionQuery) {
      return {$and: [query, instancePermissionQuery]};
    }
  ...
  canUser(permissions, user) {
    ...
    // Does user have a class-level permission?
    if (User.hasPermission(permissions, user)) {
      // TODO: Check for all permissions which end with "_OWN" if the user is an author of the document.
      //       Only then return true for them.
      return true;
    }
    ...
    // TODO: For now everyone has no instance-level permissions.
    return false;
  }
}
